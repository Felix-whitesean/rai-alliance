                ...
                <div className="flex flex-row flex-wrap gap-4 ">
                    {images.map((img) => (
                        <div key={img.image_id} className="border p-2 rounded shadow flex flex-col gap-4 justify-between bg-[#D9D9D9]">
                            <Image src={img?.path || "/uploads/dogimage.png"} width={300} height={330} alt={img.filename} className="object-cover" />
                            <div className="flex flex-col self-start gap-2">
                                <Button
                                    onClick={() => {navigator.clipboard.writeText(img.image_id.toString()); setCopied(true)} }
