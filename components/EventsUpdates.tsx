...
const EventsUpdates = () => {
    ...
    useEffect(() => {
        const fetchRole = async () => {
            setLoading(true);
            try {
                const res = await fetch("/api/user/check_role");
            ...
            <div className="bg-background w-full sm:p-16 p-4 sm:pt-48 pt-64">
                ...
                {loading ? (
                    ...
                    events.map((event) => {
                        ...
                        return (
                            <div key={event.event_id} className="event text-foreground w-full flex lg:flex-row flex-col gap-16 justify-between items-center py-32 mt-8 lg:px-24">
                                    ...
                                    <div className="section_hero flex flex-col gap-8">
                                        <div className="section_heading gap-2 flex flex-col">
                                            <div className="section_title flex flex-row gap-4">
                                                <h3 className="text-foreground font-bold text-[1.2rem] max-w-64 overflow-x-hidden truncate">{event.fullname ? event.fullname : "Check soon"}</h3>
                                                <div className="bg-foreground text-background p-1 rounded-sm text-[0.75rem] self-center">{event.country_of_residence ? event.country_of_residence  : "undefined"}</div>
                                            </div>
                                            <div className="icons flex flex-row gap-2">
                                                { event.linkedin ?
                                                        <Link href= {event.linkedin}>
                                                            <FaLinkedin size={22} color="#0077B5" />
                                                        </Link>
                                                    : ""}
                                                { event.email ?
                                                    <Link href={"mailto:"+event.email}>
                                            ...
                                            {event.company ?
                                                ...
                                                <span className="text-prim-color font-medium">{event.position_in_company ? event.position_in_company+", " : "undefined"}</span>{event.company}</Link>
                                                :""}
                                        </div>
